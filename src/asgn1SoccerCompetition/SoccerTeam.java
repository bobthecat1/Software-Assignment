...
public class SoccerTeam implements SportsTeam, Comparable<SoccerTeam>{
	...
	public SoccerTeam(String official, String nick) throws TeamException{
		if(official == "" || nick == ""){
			throw new TeamException("Offical or nick name empty");
		}
	...
	public void displayTeamDetails(){
		System.out.println(officialName  + '\t' + nickName + '\t' + form.toString() + '\t' + (+ matchesWon + matchesLost + matchesDrawn) + '\t' + matchesWon + '\t' + matchesLost + '\t' + matchesDrawn +'\t' + goalsScoredSeason + '\t' + goalsConcededSeason + '\t' + this.getGoalDifference() + '\t' + competitionPoints);
	}	
	 ...
	 */
	//throws an exception if any of the goals are unrealistic numbers
	//adds goals to class variables,
	//Checks if the team won,loss or drawn the match, then. 
	//Adds char,competition points and matches class variable that represents the results, 
	public void playMatch(int goalsFor, int goalsAgainst) throws TeamException{
		...
		if(goalsFor < 0 || goalsFor > 20 || goalsAgainst < 0 || goalsAgainst > 20){
			...
			throw new TeamException("the goal scores are unrealistic numbers");
